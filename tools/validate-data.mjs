...
/**
 * Validate the genealogy corpus under `repoRoot/data/genealogies`.
 * Returns `{ ok, errors, count }`. Does not set process.exitCode.
 */
export function validateCorpus(repoRoot) {
  const dataDir = path.join(repoRoot, "data", "genealogies");
  const errors = [];
  const fail = (msg) => {
    errors.push(msg);
  };
...
if (isMain) {
  ...
  console.log(`validate-data: ok (${result.count} genealogies)`);
}
